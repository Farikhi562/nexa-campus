...
-- ----------------------------------------------------------------------------
-- ----------------------------------------------------------------------------
-- STORAGE: bucket foto profil (dibungkus supaya kegagalan di sini TIDAK
-- membatalkan seluruh script — bagian kritis di atas tetap tersimpan).
-- ----------------------------------------------------------------------------
do $storage$
begin
  begin
  ...
  begin
    execute $p$drop policy if exists "Profile photos are publicly readable" on storage.objects$p$;
    execute $p$create policy "Profile photos are publicly readable" on storage.objects for select using (bucket_id = 'profile-photos')$p$;
  exception when others then null; end;

  begin
    execute $p$drop policy if exists "Users can upload own profile photos" on storage.objects$p$;
    execute $p$create policy "Users can upload own profile photos" on storage.objects for insert to authenticated with check (bucket_id = 'profile-photos' and (storage.foldername(name))[1] = auth.uid()::text)$p$;
  exception when others then null; end;

  begin
    execute $p$drop policy if exists "Users can update own profile photos" on storage.objects$p$;
    execute $p$create policy "Users can update own profile photos" on storage.objects for update to authenticated using (bucket_id = 'profile-photos' and (storage.foldername(name))[1] = auth.uid()::text) with check (bucket_id = 'profile-photos' and (storage.foldername(name))[1] = auth.uid()::text)$p$;
  exception when others then null; end;

  begin
    execute $p$drop policy if exists "Users can delete own profile photos" on storage.objects$p$;
    execute $p$create policy "Users can delete own profile photos" on storage.objects for delete to authenticated using (bucket_id = 'profile-photos' and (storage.foldername(name))[1] = auth.uid()::text)$p$;
  exception when others then null; end;
end $storage$;
