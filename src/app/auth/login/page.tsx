...
function humanizeAuthError(message: string) {
  ...
  if (lower.includes('invalid login credentials')) {
    return 'Email/password salah, atau email baru belum dikonfirmasi. Kalau baru daftar, cek inbox/spam lalu klik link konfirmasi dari Supabase.'
  }
  if (lower.includes('email not confirmed') || lower.includes('not confirmed')) {
    return 'Email belum dikonfirmasi. Cek inbox/spam, klik link konfirmasi, lalu login lagi.'
  }
  if (lower.includes('signup') && lower.includes('disabled')) {
    return 'Pendaftaran email/password belum aktif di Supabase Auth. Aktifkan Email provider di dashboard Supabase.'
  }
...
export default function LoginPage() {
  ...
  async function handlePasswordAuth(e: React.FormEvent) {
    ...
    if (isSignup) {
      ...
      setMessage('Akun dibuat. Kalau Supabase minta konfirmasi, cek email kamu dulu lalu login dengan password ini.')
      setLoading(false)
      return
    }
    ...
    if (data.user) {
      const completed = await getProfileCompletion(data.user.id)
  ...
  async function handleMagicLink(e: React.FormEvent) {
    ...
    if (otpError) {
      ...
      setMessage(`Magic link dikirim ke ${email.trim()}.`)
    }
  }
        ...
        <main className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-10 text-slate-950">
          <div className="w-full max-w-md">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xl shadow-slate-200/70">
              ...
              <div className="mb-5 grid grid-cols-3 rounded-xl bg-slate-100 p-1 text-sm font-semibold">
                {[
              ...
              <button
                ...
                className="mb-5 flex w-full items-center justify-center gap-3 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {googleLoading ? (
              ...
              <div className="relative mb-5">
                ...
                <div className="relative flex justify-center text-xs">
                  <span className="bg-white px-3 font-semibold uppercase tracking-wide text-slate-400">
                    atau
                  </span>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="muhamad.fauzan@gmail.com"
                      className="w-full rounded-xl border border-slate-300 py-3 pl-11 pr-4 text-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
                      required
                    />
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full rounded-xl border border-slate-300 py-3 pl-11 pr-12 text-sm outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
                        required
                      />
                      <button
                        type="button"
              ...
              <p className="mt-5 text-center text-xs leading-5 text-slate-500">
                Dengan masuk, kamu bisa memakai fitur belajar gratis. Fitur jual barang dan jasa dibuka untuk paket Basic dan Pro.
              </p>
            </div>
          </div>
        </main>
