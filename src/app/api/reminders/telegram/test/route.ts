...
export async function POST(request: NextRequest) {
  ...
  if (!token) {
    return NextResponse.json(
      {
        error:
          'TELEGRAM_BOT_TOKEN belum tersedia di server. Cek .env.local lalu restart dev server.',
      },
      { status: 400 }
    )
  }
  ...
  const response = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
    ...
    body: JSON.stringify({
      ...
      text: 'Halo dari NEXA Campus. Telegram reminder kamu sudah nyambung. Deadline tetap kamu yang kerjain, tragis tapi adil.',
    ...
    return NextResponse.json(
      {
        error:
          result?.description ||
          'Telegram menolak pesan test. Cek chat ID dan pastikan sudah start bot.',
      },
      { status: 502 }
    )
  }

  await supabase.from('profiles').update({ telegram_chat_id: telegramChatId }).eq('id', user.id)
