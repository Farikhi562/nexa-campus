    ...
    return {
      ...
      course:
        typeof source.course === 'string'
          ? source.course
          : typeof source.course_name === 'string'
            ? source.course_name
            : null,
...
export async function POST(request: NextRequest) {
  ...
  if (!user) {
    return NextResponse.json(
      { error: 'Kamu perlu login dulu untuk pakai Tanya NEXA.' },
      { status: 401 }
    )
  }
  ...
  if (question.length > MAX_QUESTION_LENGTH) {
    return NextResponse.json(
      { error: 'Pertanyaannya kepanjangan. Coba ringkas maksimal 1000 karakter.' },
      { status: 400 }
    )
  }

  const userContext =
    typeof body.userContext === 'object' && body.userContext !== null
