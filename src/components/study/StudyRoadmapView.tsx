...
export default function StudyRoadmapView({ roadmap, packId }: { roadmap: StudyRoadmapStep[]; packId: string }) {
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {roadmap.map((s) => {
          ...
          return (
            <div key={s.step} className={`rounded-2xl border p-3 transition ${isDone ? 'border-emerald-200 bg-emerald-50/60' : 'border-slate-200 bg-white'}`}>
              <button type="button" onClick={() => toggle(s.step)} className="flex w-full items-start gap-3 text-left">
                {isDone ? (
              ...
              <div className="ml-8">
                <ConceptExplainer packId={packId} concept={s.title} description={s.description} />
              </div>
            </div>
          )
        })}
