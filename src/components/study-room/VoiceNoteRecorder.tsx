  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-base font-black text-slate-950">Voice Note</h3>
          <p className="text-sm text-slate-500">Rekam VN buat Study Room. Maks 10MB, Command-only.</p>
        </div>
        <div className="text-sm font-black text-blue-600">{formatSeconds(duration)}</div>
      </div>
        ...
        {audioBlob ? (
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={uploading}
              className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 px-4 py-3 text-sm font-black text-slate-700 transition hover:bg-slate-50 disabled:opacity-50"
            >
      ...
      {error ? <p className="mt-3 rounded-2xl bg-rose-50 px-3 py-2 text-sm font-semibold text-rose-700">{error}</p> : null}
    </div>
  )
}
