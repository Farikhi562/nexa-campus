...
export default function RetentionFomoStrip({
  ...
  const referralLabel = referralCount >= 25 ? 'NEXA Origin sudah terbuka' : `${remainingReferral} referral lagi menuju milestone`
        ...
        <div className="relative flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <Badge tone="warning" className="bg-amber-300/15 text-amber-100 ring-amber-200/20">Profil & badge</Badge>
              <Badge tone="info" className="bg-cyan-300/15 text-cyan-100 ring-cyan-200/20">Update v1.5.23</Badge>
            </div>
            <h2 className="mt-3 text-xl font-black tracking-tight sm:text-2xl">
              Lengkapi profil, kumpulkan badge, dan pantau progresmu.
            </h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">
              NEXA sekarang punya progress, referral, badge rarity, dan profil publik supaya aktivitas belajar terasa lebih hidup.
            </p>
          </div>
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:min-w-[420px]">
            <Link href="/dashboard/achievements" className="group rounded-2xl border border-white/10 bg-white/10 p-3 transition hover:-translate-y-0.5 hover:bg-white/15">
              <div className="flex items-center gap-2 text-amber-100"><Crown className="h-4 w-4" /><span className="text-xs font-black uppercase">Badge langka</span></div>
              <p className="mt-1 text-sm font-black text-white">NEXA Origin</p>
              <p className="mt-1 text-[11px] leading-4 text-slate-300">Badge khusus dengan animasi aktif.</p>
            </Link>
            <Link href="/dashboard/achievements" className="group rounded-2xl border border-white/10 bg-white/10 p-3 transition hover:-translate-y-0.5 hover:bg-white/15">
              ...
              <p className="mt-1 text-sm font-black text-white">Badge yang bisa ditampilkan</p>
              <p className="mt-1 text-[11px] leading-4 text-slate-300">Epic badge dibuat terbatas untuk pencapaian tertentu.</p>
            </Link>
            <Link href="/dashboard" className="group rounded-2xl border border-white/10 bg-white/10 p-3 transition hover:-translate-y-0.5 hover:bg-white/15">
              ...
              <p className="mt-1 text-sm font-black text-white">{todayActiveCount > 0 ? `${todayActiveCount} deadline aktif` : 'Daily Pulse dulu'}</p>
              <p className="mt-1 text-[11px] leading-4 text-slate-300">{highPriorityCount > 0 ? `${highPriorityCount} prioritas tinggi perlu dilihat.` : 'Jaga streak dengan check-in harian.'}</p>
            </Link>
            <Link href="/dashboard#referral" className="group rounded-2xl border border-white/10 bg-white/10 p-3 transition hover:-translate-y-0.5 hover:bg-white/15">
              ...
              <p className="mt-1 text-[11px] leading-4 text-slate-300">Paket kamu: {userTier.toUpperCase()} · reward Pulse otomatis.</p>
            </Link>
          </div>
        </div>
      </CardContent>
