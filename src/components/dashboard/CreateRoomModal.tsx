...
const CATEGORIES = [
  'umum','informatika','matematika','fisika','kimia','biologi',
...
export default function CreateRoomModal({
  ...
  const [form, setForm] = useState({
    title: '', description: '', topic: '', category: 'umum',
    max_members: '8', visibility: 'public', scheduled_at: '',
  })
  ...
  async function submit() {
    if (!form.title.trim()) { setError('Judul room wajib diisi.'); return }
    ...
    setLoading(false)
    if (!res.ok) { setError(json.error ?? 'Gagal membuat room.'); return }
    onCreated()
  }

  const inputCls = 'focus-ring w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400'
    ...
    <div className="fixed inset-0 z-[70] flex items-end justify-center sm:items-center" onClick={onClose}>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          ...
          <button onClick={onClose} className="focus-ring rounded-xl p-1.5 text-slate-500 hover:bg-slate-100">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input value={form.title} onChange={(e) => set('title', e.target.value)} placeholder="Cth: Belajar Kalkulus UTS" className={inputCls} />
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              ...
              <input type="number" min="2" max="50" value={form.max_members} onChange={(e) => set('max_members', e.target.value)} className={inputCls} />
            </div>
          </div>
          <div>
            <label className={labelCls}>Topik / hashtag</label>
            <input value={form.topic} onChange={(e) => set('topic', e.target.value)} placeholder="Cth: kalkulus-integral" className={inputCls} />
          </div>
          <div>
            <label className={labelCls}>Deskripsi (opsional)</label>
            <textarea value={form.description} onChange={(e) => set('description', e.target.value)} rows={3} placeholder="Tujuan room, materi yang dibahas, dll." className={inputCls} />
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
