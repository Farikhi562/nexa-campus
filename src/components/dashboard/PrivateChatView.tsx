...
import { ArrowLeft, Edit2, FileText, Loader2, Paperclip, Send, Smile, Trash2 } from 'lucide-react'
...
export default function PrivateChatView({ friendId, userId }: { friendId: string; userId: string }) {
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'private_messages' }, (payload) => {
        ...
        if (!row) return
        const involved = (row.sender_id === userId && row.receiver_id === friendId) || (row.sender_id === friendId && row.receiver_id === userId)
        if (!involved) return
  ...
  async function saveEdit() {
    ...
    const res = await fetch(`/api/chats/messages/${editingId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (res.ok && json.data) {
      ...
      setEditingId(null)
      setEditingText('')
    } else alert(json.error ?? 'Gagal edit pesan.')
  }
  ...
  async function deleteMessage(messageId: string) {
    ...
    const json = await res.json().catch(() => ({}))
    if (res.ok && json.data) setMessages((prev) => prev.map((m) => m.id === messageId ? { ...m, ...json.data } : m))
    else alert(json.error ?? 'Gagal hapus pesan.')
  }

  if (loading) return <div className="flex h-[70vh] items-center justify-center text-slate-400"><Loader2 className="h-6 w-6 animate-spin" /></div>
    ...
    <div className="flex h-[calc(100vh-8rem)] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-center text-slate-400">
            ...
            <p className="mt-1 text-xs">Mulai dari halo. Kreatif sekali, tapi memang selalu begitu.</p>
          </div>
        ) : messages.map((msg) => {
              ...
              <div className={`max-w-[76%] space-y-1 ${isMe ? 'items-end' : 'items-start'} flex flex-col`}>
                ...
                <div className={`rounded-2xl px-3.5 py-2.5 text-sm leading-6 ${isMe ? 'rounded-tr-sm bg-teal-500 text-white' : 'rounded-tl-sm bg-slate-100 text-slate-900'}`}>
                  {deleted ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2"><button onClick={saveEdit} className="rounded-xl bg-slate-950 px-3 py-1 text-xs font-black text-white">Simpan</button><button onClick={() => setEditingId(null)} className="rounded-xl bg-white/80 px-3 py-1 text-xs font-black text-slate-700">Batal</button></div>
                    </div>
                ...
                {isMe && !deleted && (
                  <div className="flex gap-1 opacity-80">
                    {msg.message_type === 'text' && <button onClick={() => { setEditingId(msg.id); setEditingText(msg.content ?? '') }} className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black text-slate-600 hover:bg-slate-200"><Edit2 className="inline h-3 w-3" /> Edit</button>}
                    <button onClick={() => deleteMessage(msg.id)} className="rounded-full bg-red-50 px-2 py-1 text-[10px] font-black text-red-600 hover:bg-red-100"><Trash2 className="inline h-3 w-3" /> Hapus</button>
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="flex-shrink-0 border-t border-slate-100 p-3">
        <div className="mb-2 flex flex-wrap gap-1">
          {EMOJIS.map((emoji) => <button key={emoji} type="button" onClick={() => setMessage((v) => `${v}${emoji}`)} className="rounded-xl bg-slate-100 px-2 py-1 text-sm hover:bg-slate-200">{emoji}</button>)}
        </div>
        <div className="flex items-end gap-2">
          <input ref={fileInputRef} type="file" accept="image/*,video/mp4,video/webm,video/quicktime,.pdf,.doc,.docx,.xls,.xlsx,.txt,.zip" className="sr-only" onChange={(e) => { const f = e.target.files?.[0]; if (f) void sendFile(f); e.target.value = '' }} />
          ...
          <textarea value={message} onChange={(e) => setMessage(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); void sendMessage() } }} placeholder="Tulis chat pribadi..." rows={1} className="focus-ring flex-1 resize-none rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm" />
